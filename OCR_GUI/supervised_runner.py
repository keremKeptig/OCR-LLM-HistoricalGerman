...
def train_model_from_error_rates(input_dir: Path,
    ...
    if df.empty:
        raise RuntimeError("No overlap between error-rate pages")
...
def score_books(book_dirs, model, embedder, outdir: Path):
    ...
    outdir = Path(outdir)
    pagescores_csv = outdir / "pagescores.csv"
    summaries_csv = outdir / "book_summaries.csv"

    #if we already have outputs
    if pagescores_csv.exists() and summaries_csv.exists():
        print(f"Loading existing scores from {pagescores_csv}")
        return pd.read_csv(pagescores_csv)
...
def run_pipeline_supervised_model(
    gt_dir,
    input_dir,
    labels_jsonl,
    page_error_rates_csv,
    model_path,
    outdir,
    xml_outputs,
    dataset_path,                
    score_book_dirs=None       
    ):

    gt_dir = Path(gt_dir)
    input_dir = Path(input_dir)
    labels_jsonl = Path(labels_jsonl)
    page_error_rates_csv = Path(page_error_rates_csv)
    model_path = Path(model_path)
    outdir = Path(outdir)
    xml_outputs = Path(xml_outputs)
    dataset_path = Path(dataset_path)
    score_book_dirs = [Path(p) for p in (score_book_dirs or [input_dir])]

    # labels + page error rates (skip if exist)
    if not labels_jsonl.exists():
        print("Generating word-level labels…")
        generate_word_level_labels(gt_dir, input_dir, labels_jsonl)
        compute_page_error_rates(labels_jsonl, page_error_rates_csv)
    else:
        print(f"Using existing labels: {labels_jsonl}")
        if not page_error_rates_csv.exists():
            compute_page_error_rates(labels_jsonl, page_error_rates_csv)

    # Skip model if XMLs already exist
    existing_xmls = list(xml_outputs.glob("*.xml"))
    if existing_xmls:
        print(f"Found existing XMLs, skipping model training.")
        return {
            "predicted_xml_count": len(existing_xmls),
            "pages_scored": None,
            "xml_out": str(xml_outputs),
            "scores_out": None,
            "model_path": str(model_path if model_path.exists() else "N/A")
        }
    ...
    if model_path.exists():
        ...
        regressor, embedder = train_model_from_error_rates(
            input_dir=input_dir,
            error_rates_csv=page_error_rates_csv,
            model_path=model_path
        )

    # write predicted XMLs to your viewer folder
    pred_scorer = FinalRegressorPageScorer(regressor, embedder)
    pred_pages = calculate_annotated_pages_with_pred_scorer(dataset_path, pred_scorer)
    xml_outputs.mkdir(parents=True, exist_ok=True)
    written = save_pages(pred_pages, xml_outputs)
    print(f"Wrote {len(written)} predicted XMLs -> {xml_outputs}")

    # book scores, pagescores.csv + book_summaries.csv 
    outdir.mkdir(parents=True, exist_ok=True)
    df = score_books(score_book_dirs, regressor, embedder, outdir)
    print(f"Book scores -> {outdir}")

    return {
        "predicted_xml_count": len(written),
        "pages_scored": int(len(df) if df is not None else 0),
        "xml_out": str(xml_outputs),
        "scores_out": str(outdir),
...
if __name__ == "__main__":
    ...
    # books to score after training (each folder as one book for further development)
    SCORE_BOOK_DIRS = [
    ...
    # Word-level labels + page error rates (only if missing)
    if not labels_jsonl.exists():
        generate_word_level_labels(gt_dir, input_dir, labels_jsonl)
